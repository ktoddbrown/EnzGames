...
parm.ls <- list(b=c(-10, -1, -0.1, 0.1, 1, 10),
                ...
                C=c(10, 50, 100, 200, 300),
                #kmin_rel = c(0.01, 0.05, 0.1, 0.5, 1), #assume kmin relative to C
                #kmax_rel = c(1, 5, 10, 50, 100), #assume kmax relative to C
                cue=seq(0.01, 0.99, length=100)
                )
tradeoff.df <- expand.grid(parm.ls)

tradeoff.df$kmin <- 0
tradeoff.df$k_v_slope <- with(tradeoff.df, (C*kmax_rel-kmin)/vmax)

tradeoff.df$v <- cue_v_tradeoff(b=tradeoff.df$b, vmax=tradeoff.df$vmax, cue=tradeoff.df$cue)
#tradeoff.df$k <- v_k_tradeoff(kmin=tradeoff.df$kmin, v=tradeoff.df$v, k_v_slope=tradeoff.df$k_v_slope)

#tradeoff.df$stable <- with(tradeoff.df, v*cue*(k+C))
tradeoff.df$index <- 1:(dim(tradeoff.df)[1])

#ggplot(tradeoff.df) + geom_histogram(aes(x=stable)) + scale_x_log10()
#ggplot(tradeoff.df) + geom_point(aes(x=cue, y=stable, color=k)) + facet_wrap(kmin_rel+kmax_rel~vmax+b+C)

strategic_pairs <- ddply(tradeoff.df, c('index'), function(xx){
  temp <- subset(tradeoff.df, index != xx$index & 
                   b==xx$b & vmax==xx$vmax & kmin==xx$kmin & k_v_slope==xx$k_v_slope)
  ##Calculate tradeoff-slope based on C, then set stable
  
  bestMatch <- temp[which.min(abs(xx$stable-temp$stable)),]
  if(bestMatch$index < xx$index){
    return(data.frame())
  }
  return(data.frame(matchIndex = bestMatch$index, 
                    rel_drift=abs(xx$stable-bestMatch$stable)/xx$stable))
})

ggplot(strategic_pairs) + geom_histogram(aes(x=rel_drift)) + scale_x_log10()

temp <- merge(strategic_pairs, tradeoff.df)
temp$index.x <- temp$index
temp$index <- temp$matchIndex
temp <- merge(temp, tradeoff.df, by=c('index', 'b', 'kmin_rel', 'kmax_rel', 'vmax', 'C'))
temp$C.ss <- with(temp, (v.x*cue.x*k.y-v.y*cue.y*k.x)/(v.y*cue.y-v.x*cue.x))
ggplot(subset(temp, abs(C.ss-C)/C < 0.1)) + geom_point(aes(x=cue.x, y=cue.y, 
                              color=as.factor(b), shape=as.factor(C))) +
  geom_point(aes(y=cue.x, x=cue.y, 
                              color=as.factor(b), shape=as.factor(C))) +
  geom_abline(slope=1, intercept=0) + facet_wrap(kmax_rel~kmin_rel)

ggplot(temp) + geom_point(aes(x=stable.x, y=stable.y)) + geom_abline(slope=1, intercept=0)
ggplot(temp) + geom_point(aes(x=C, y=(v.x*cue.x*k.y-v.y*cue.y*k.x)/(v.y*cue.y-v.x*cue.x))) +
  geom_abline(slope=1, intercept=0)

summary(temp[abs(temp$C - with(temp, (v.x*cue.x*k.y-v.y*cue.y*k.x)/(v.y*cue.y-v.x*cue.x)))/temp$C < 0.1,])
